...
import { listAgents } from "../_lib/book.js";

/**
 * Staff records visible to the caller.
 *
 ...
 * endpoint requires a session, and agentScope decides which rows exist for
 * that session. It fails closed: an unrecognised role resolves to no rows.
 */
export default withErrors(async (req: VercelRequest, res: VercelResponse) => {
  requireMethod(req, ["GET"]);
  const session = await requireUser(req);
  send(res, 200, { agents: await listAgents(session) });
});
